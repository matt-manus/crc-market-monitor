...
- [ ] 比較參考圖與 Massive 真實輸出使用的股票宇宙、日線版本、指數來源及指標分母，清楚記錄每項讀數的口徑差異。
- [ ] 將 4% 動量脈搏圖的跌幅資料改為零線下方的負值柱，加入 800、+300、0、−300、−800 縱軸標籤及基準虛線。
- [ ] 由 SEC 公司申報／SIC 資料建立可重現的代號至行業映射，計算每個真實行業的領導股佔比、池內比率、滲透率及超額百分點。
- [ ] 將修正後的真實資料、公開頁面及每日同步程序重新部署至 GitHub Pages。
- [ ] 以瀏覽器驗證更新後的圖表方向、座標和行業表，再向使用者交代資料差異是否屬計算法或資料供應範圍所致。
